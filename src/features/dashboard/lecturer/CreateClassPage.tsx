...
export const CreateClassPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!user) return;
        if (!formData.code || !formData.title || !formData.department || !formData.level || formData.scheduleDays.length === 0 || !formData.scheduleTime) {
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 max-w-3xl">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {daysOfWeek.map(day => (
                                    <button 
                                        ...
                                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${formData.scheduleDays.includes(day) ? 'bg-primary text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                                    >
                                        {day}
                                    </button>
                                ))}
                        ...
                        <div className="pt-4">
                            <button disabled={isLoading} type="submit" className={`px-8 py-3.5 bg-primary text-white font-bold rounded-xl hover:bg-primary-hover transition-all shadow-md flex items-center justify-center min-w-[200px] ${isLoading ? 'opacity-70' : ''}`}>
                                {isLoading ? <span className="material-symbols-outlined animate-spin mr-2">progress_activity</span> : 'Create Class'}
                            </button>
                        </div>
                    </form>
                </div>
